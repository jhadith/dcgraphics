  ...
  return (
    <>
              ...
              >
                ...
                <div className="mt-4 flex items-center justify-between gap-4">
                  <div>
                    ...
                    <h2 className="mt-1 font-display text-2xl font-black text-ink">{item.topic}</h2>
                  </div>
      ...
      {workTopics.map((topic) => (
        <section key={topic.slug} className="py-14 lg:py-20">
          <div className="container-page">
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-sm font-bold uppercase tracking-[0.18em] text-accent">Project category</p>
                <h2 className="mt-3 font-display text-4xl font-black leading-[1.04] text-ink sm:text-5xl">{topic.label}</h2>
              </div>
              <Link className="focus-ring rounded-md bg-ink px-4 py-2 text-sm font-black text-white transition hover:bg-accent hover:text-navy" to={`/${topic.slug}`}>
